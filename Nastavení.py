import flet as ft
from flet import Text, TextField, ElevatedButton, Switch
import os
...
class ResetHesla(ft.UserControl):
    ...
    def submit_click(self, e):
        ...
        password, salt = hash_password(self.password_confirm.value)

        cursor.execute("UPDATE user SET email = %s, password = %s, salt = %s WHERE id = %s", (self.email.value, password, salt, self.session.get("user_id")))
