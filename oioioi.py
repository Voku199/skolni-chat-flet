# Python a atd..
import flet as ft
from flet import Text, TextField, Dropdown, ElevatedButton, Row, Checkbox, FilePicker
...
for milestone in milestones:
    cursor.execute("INSERT INTO achievements (name, description, criteria) VALUES (%s, %s, %s)",
                   (f"{milestone} Messages", f"Send {milestone} messages", milestone))
mydb.commit()
...
class Registrace:
    def __init__(self, Page: ft.Page):
        ...
        self.souhlas_pravidla = Checkbox(
            label="Souhlasíš s pravidlama?"
        )
        self.show_pravidla_button = ElevatedButton(
            text="Zobrazit pravidla", on_click=self.show_pravidla
        )
        self.submit = ElevatedButton(
            text="Registrovat", on_click=lambda e: self.submit_click(Page, e)
        )
        self.error_message = Text(
            "Jestli se ty nejde zaregistrovat, tak zkus jiný jméno, třeba přidat za svým jménem tečku, čárku, nebo něco jiného. Někdo už bohužel má tohle jméno zabraný.",
            size=11,
        )
        self.page = Page
    ...
    def show_pravidla(self, e):
        # Otevřít dialogové okno s pravidlya
        if (
                not self.user_name.value
                or not self.email.value
                or not self.password.value
...
class ChatMessage(ft.Row, str):
    def __init__(self, message: Message):
        ...
        avatar_with_click = ft.GestureDetector(  # Zde jsem přidal GestureDetector pro zpracování kliknutí
            content=avatar,
            on_tap=lambda e: self.on_avatar_click(e, message.user_id)  # Zde jsem předal message.user_id
        )
    ...
    def on_avatar_click(self, event, user_id):
        # Zde definujete, co se má stát při kliknutí na avatar
    ...
    def execute(self, page, user_id):
        ...
        if user_id and result:
            ...
            user_text = f"Uživatel s ID '{user_id}' nebyl nalezen."

        # Vytvoření a otevření dialogu s informacemi o uživateli
        page.dialog = ft.AlertDialog(
            title=ft.Text("Informace o uživateli"),
            content=ft.Text(user_text),
            on_dismiss=lambda e: print("Dialog dismissed")
        )
        page.dialog.open = True
        page.update()
...
def _login(username: TextField, password: TextField) -> dict:
    ...
    user = get_user_by_username(username)  # Fetch user from the database
    if user and user.password == password:
        if user.is_banned:
            return {"success": False, "message": "You are banned from this chat."}
...
def main(page: ft.Page, ):
    ...
    def join_chat_click(e):
        ...
        print(f"Uživatel s ID před přihlášením: {user_id}")
        print(f"Uživatel s User_Name před přihlášením: {user_name}")

        if page is not None:
            if isinstance(text_username, ft.TextField) and isinstance(
        ...
        if not text_username.value or not text_password.value:
            ...
            page.dialog.actions.append(
                ft.ElevatedButton(
                    text="Zapomenuté údaje?", on_click=clear_error_and_retry
                )
            )
            return
        ...
        if not login["success"]:
            ...
            print(f"Uživatel s ID po přihlášení: {login['user']['id']}")
            print(f"Uživatel s User_Name před přihlášením: {login["user"]["user_name"]}")
            ...
            error_dialog = ft.AlertDialog(
                ...
                content=ft.Column(
                    [
                        ...
                        ElevatedButton(
                            text="Zpět k chatu", on_click=lambda e: zpatky(page, chat)
                        ),
                    ],
                    width=200,
                    height=200,
    ...
    def pick_files_result(e: ft.FilePickerResultEvent):
        page = e.page  # Získání instance stránky z události
        user_name = page.session.get("user_name")

        if e.files:
            pick_files_dialog.value = f"Vybraný soubor: {e.files[0].name}"
            vybrat = f"Uživatel/ka {user_name} si vybral/a soubor s názvem: {e.files[0].name}"

            with open(e.files[0].name, "rb") as file:
                file_content = file.read()
                file_base64 = base64.b64encode(file_content).decode("utf-8")

            # Vytvoření tlačítka pro stažení souboru
            ick = ft.Icon(name=ft.icons.FAVORITE, color="pink" f"{file_base64}"),

            download_button = ft.ElevatedButton(
                text="Stáhnout soubor",
                on_click=lambda _: webbrowser.open_new_tab(f"data:application/octet-stream;base64,{file_base64}"),
            )

            vybrat_with_button = f"{vybrat}  .  {download_button}"

            chat.controls.append(
                current_page.pubsub.send_all(
                    Message(
                        user_name="Zib",
                        text=vybrat_with_button,
                        message_type="chat_message",
                        user_role="Bot",
                        page=page,
                    )
    ...
    pick_files_dialog = ft.FilePicker(on_result=partial(pick_files_result))
    ...
    def handle_help(message: Message, page: ft.Page, ):
        ...
        help_text += "- !user [Id nebo jeho Jméno] : Vyhledá ty to\n"
        help_text += "- !m [Cokoliv] : Tenhle příkaz funguje tak že vyhledáš všechny zprávy ohledně tý zprávy\n"
        help_text += "- !av : Tenhel příkaz funguje tak, že ty dá všechny možné achivmenty, A když je budeš mít hotové, tak vedle toho budeš mít napsáno Hotovo.\n"
        ...
        if message.user_role in full_permissions:
            ...
            help_text += "-!m [Jakýkoliv slovo] : Vyhledá to slovo který ten danný uživatel napsal např : !m Ahoj. Vyhledá to všechny zprávy ohledně Ahoj.\n "
    ...
    def ban_user(user_id, duration_minutes, reason=""):
        try:
            ...
            cursor.execute(
                ...
                (user_id, banned_end_time, reason),
            )
            mydb.commit()
            print(
                f"Uživatel {user_id} byl zabanován na {duration_minutes} minut." if duration_minutes != -1 else f"Uživatel {user_id} byl zabanován na neomezenou dobu.")
    ...
    user_permissions = ["Majitel", "Spolu Majitel", "Admin", "Učitel", None, "Test", "Ředitel"]
    full_permissions = ["Majitel", "Spolu Majitel", "Admin", "Učitel", "Test", "Ředitel"]
