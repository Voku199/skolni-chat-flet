...
achievements = [
    {
        "name": "První chat",
        "description": "Dokončil jste svůj první chat.",
        "condition": 1,
    },
    ...
    {"name": "Sto chatů", "description": "Dokončil jste 100 chatů.", "condition": 100},
    {
        ...
        "description": "Pravidelně se účastníte chatů po dobu jednoho týdne.",
        "condition": 7,
    },  # Example condition
...
class Registrace:
    def __init__(self, Page: ft.Page):
        ...
        self.souhlas_pravidla = Checkbox(
            label="Souhlasíš s pravidly který jsou na školní chat?"
        )
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def show_user_details(message, page):
        ...
        if user_identifier.isdigit():
            cursor.execute(
                "SELECT user_name, class, role, id, email FROM user WHERE id = %s LIMIT 1",
                (user_identifier,),
            )
        else:
            cursor.execute(
                "SELECT user_name, class, role, id, email FROM user WHERE user_name = %s LIMIT 1",
                (user_identifier,),
            )
    ...
    def process_new_message(message: Message, chat, page: ft.Page):
        ...
        if new_achievements:
            achievements_text = (
                "Gratulujeme! Dosáhli jste následujících achievementů:\n"
            )
            for achievement in new_achievements:
                achievements_text += (
    ...
    def on_user_message_submit(e):
        ...
        )  # Získání textu zprávy od uživatele
    ...
    def list_achievements(message: Message, page: ft.Page):
        ...
        for index, achievement in enumerate(achievements, start=1):
            achievements_text += (
                f"{index}. {achievement['name']}: {achievement['description']}\n"
            )
