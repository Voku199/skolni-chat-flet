...
class Nastavení(ft.UserControl):
    def __init__(self, page):
        ...
        self.bio_textbox = ft.TextField(label="Napiš jsi vlastní bio")
        self.submit_button = ft.ElevatedButton(text="Uložit jsi bio", on_click=self.save_bio_to_db)
        ...
        self.reset_password_button = ElevatedButton(
            text="Restartovat heslo", on_click=self.reset_password_click
        )
    ...
    def build(self):
        ...
        controls = [
            ...
            ft.Text("", size=15),
            ft.Text("Resetuj jsi heslo:", size=20),
            self.reset_password_button,
            ft.Text("", size=15),
            ft.Text("Napiš jsi vlastní bio!", size=20),
            self.bio_textbox,
            self.submit_button,
            ft.Text("", size=10),
            ...
            self.message_placeholder, ft.Text(
                "Zde máš profilovku:",
                max_lines=20),
            profile_picture,
    ...
    def save_bio_to_db(self, e):
        user_id = self.page.session.get("user_id")
        bio_text = self.bio_textbox.value.strip()

        if user_id and bio_text:
            try:
                # Uložení bio do databáze
                cursor.execute("UPDATE user SET bio = %s WHERE id = %s", (bio_text, user_id))
                mydb.commit()
                self.message_placeholder.controls = [
                    ft.Text("Bio bylo úspěšně uloženo :O.", color=ft.colors.GREEN_500)
                ]
                self.page.update(self.message_placeholder)
            except mysql.connector.Error as err:
                print(f"Nastala chyba: {err}")
                self.message_placeholder.controls = [
                    ft.Text(f"Nastala chyba při ukládání bio {e}.", color=ft.colors.RED_500)
                ]
                self.page.update(self.message_placeholder)
        else:
            self.message_placeholder.controls = [
                ft.Text("Prosím, vyplňte vaše bio >:(.", color=ft.colors.RED_500)
            ]
            self.page.update(self.message_placeholder)
    ...
    def file_picker_result(self, e: ft.FilePickerResultEvent):
        ...
        if e.files:
            ...
            if selected_file.name.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                error_text = ft.Text(
                    "Vybraný soubor není obrázek >:/. Vyberte prosím soubor ve formátu PNG, JPG nebo JPEG.",
    ...
    def save_profile_picture_to_db(self, base64_data):
        ...
        try:
            ...
            success_text = ft.Text(
                "Profilový obrázek byl úspěšně uložen do databáze :O. Když se přihlásíte znovu. Obrázek by se měl zobrazit. Stejně jako u aktualizace.",
                max_lines=20,
...
class ResetHesla(ft.UserControl):
    def __init__(self, page):
        ...
        self.error_message = Text("", color=ft.colors.RED_500, size=12)
    ...
    def submit_click(self, e):
        if (
            ...
            self.error_message.color = ft.colors.RED_500
        ...
        if result:
            self.error_message.value = "Heslo změněno :O."
            self.error_message.color = ft.colors.GREEN_500
            self.error_message.update()
            print("Bylo")
        else:
            self.error_message.value = f"Nastala chyba (Jak?). {e}"
            self.error_message.color = ft.colors.RED_500
            self.error_message.update()
            print("Nebylo")
