...
def send_email(to_email, subject, body):
    ...
    try:
        # Připojení k SMTP serveru a odeslání emailu
...
def on_verify():
    ...
    if send_email(user_email, email_subject, email_body):
        messagebox.showinfo("Úspěch", f"Ověřovací kód byl odeslán na {user_email}")
        show_code_entry()
    else:
...
verify_button = tk.Button(root, text="Ověřit", command=on_verify)
